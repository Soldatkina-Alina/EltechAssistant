...
db = pg_driver.connect(user='postgres', password='db!StAr_1((7', host='localhost', database='postgres', port=5432)
...
class Database:
    ...
    def shedule (text):
        ...
        if 'Все расписание' in text:
            ...
            return Prettyfier.shedule(data)
            ...
            if number_week == 1:
                print(number_week)
                data = db.query("WITH p AS (SELECT * FROM TimetableBoth WHERE DayOfWeek =" + number_day + " UNION (SELECT * FROM TimetableFirst WHERE DayOfWeek =" + number_day + ") ORDER BY DayOfWeek, LessonNumber) SELECT p.DayOfWeek, l.BeginTime, l.EndTime, p.Subject, p.Type, p.Classroom, t.Surname, t.Name, t.Patronymic FROM p LEFT JOIN Lessons l on p.LessonNumber = l.Number LEFT JOIN Teachers t on p.TeachersSurname = t.Surname and p.TeachersName = t.Name" )
                print(date)
                return Prettyfier.shedule(data)
            if number_week == 2:
                print(number_week)
                data = db.query("WITH p AS (SELECT * FROM TimetableBoth WHERE DayOfWeek ="+ number_day + " UNION (SELECT * FROM TimetableSecond WHERE DayOfWeek =" + number_day + ") ORDER BY DayOfWeek, LessonNumber) SELECT p.DayOfWeek, l.BeginTime, l.EndTime, p.Subject, p.Type, p.Classroom, t.Surname, t.Name, t.Patronymic FROM p LEFT JOIN Lessons l on p.LessonNumber = l.Number LEFT JOIN Teachers t on p.TeachersSurname = t.Surname and p.TeachersName = t.Name")
                return Prettyfier.shedule(data)
    ...
    def group(text):
        if 'Список группы' in text:
            ...
            return Prettyfier.get_read_of_none(data)
    ...
    def group_one_person(name):
        ...
        print(str(data))
        return Prettyfier.one_object(data)
    ...
    def subjects(text):
        if 'Учебный план' in text:
            ...
            print(data)
            print(str(data))
            return Prettyfier.study_plan(data)
